...
class Task {
    ...
    async run() {
       ...
       await $.wait(Math.floor(Math.random() * 20 + 5) * 1000);
        ...
        // 3. 动态获取 promotionId 并签到
        const pid = await this.getPromotionId();
        if (pid) {
            await this.doSign(pid);
        }
    }
...
async getPromotionId() {
        try {
            ...
            for (const module of res.data || []) {
                for (const item of module.detailList || []) {
                    const detailJson = JSON.parse(item.detailJson);
                    //  $.log(`detailJsondetailJsondetailJsondetailJson————————: ${detailJson.title}`);
                    if (detailJson.title && detailJson.title.includes('每日签到')) {
                        const match = detailJson.jumpData.pagePath.match(/promotionId=([^&]*)/);
                        if (match) return match[1];
                    }
                }
            }
        } catch (e) {
            $.log(`获取 promotionId 异常: ${e.message}`);
        }
        return null;
    }

    async doSign(promotionId) {
        let options = {
            method: 'GET',
            url: `https://stdcrm.dtmiller.com/scrm-promotion-service/promotion/sign/today?promotionId=${promotionId}`,
            headers: { 'Authorization': this.token }
        };
        let { data: result } = await axios.request(options);
        if (result?.code == 0) {
            $.log(`签到成功！签到天数：${result.data.signDays}`);
        } else {
            $.log(`签到失败: ${result.msg}`);
        }
    }
